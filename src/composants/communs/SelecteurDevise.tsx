...
export const SelecteurDevise: React.FC<ProprietesSelecteurDevise> = ({
        ...
        {DEVISES_DISPONIBLES.map((dev) => {
          ...
          return (
            ...
            >
              ...
              {estActif && <Icone icone="ph:check-bold" className="w-4 h-4 text-red-600 shrink-0 ml-2 mt-0.5" />}
            </button>
          );
        })}
      </div>
  ...
  if (variante === 'compact') {
    return (
      <div id={identifiant} className={`relative inline-block ${classePersonnalisee}`} ref={refMenu}>
        ...
        >
          <Icone icone="ph:coins-bold" className="w-3.5 h-3.5 text-red-600" />
          <span>{deviseActuelle} ({definitionDeviseActuelle.symbole})</span>
          <Icone icone="ph:caret-down-bold" className={`w-3 h-3 text-slate-500 transition-transform ${ouvert ? 'rotate-180' : ''}`} />
        </button>
          ...
          <div className="absolute right-0 mt-1.5 w-64 bg-white border border-slate-200 rounded-xl shadow-lg z-50 p-1.5 space-y-0.5 max-h-72 overflow-y-auto">
            ...
            {DEVISES_DISPONIBLES.map((dev) => (
              ...
              >
                ...
                {dev.code === deviseActuelle && <Icone icone="ph:check-bold" className="w-3.5 h-3.5 text-red-600" />}
              </button>
            ))}
          </div>
        )}
  ...
  return (
    <div id={identifiant} className={`relative inline-block ${classePersonnalisee}`} ref={refMenu}>
      ...
      >
        <Icone icone="ph:coins-bold" className="w-4 h-4 text-red-600" />
        ...
        <Icone icone="ph:caret-down-bold" className={`w-3.5 h-3.5 text-slate-400 transition-transform ml-1 ${ouvert ? 'rotate-180' : ''}`} />
      </button>
          ...
          {DEVISES_DISPONIBLES.map((dev) => {
            ...
            return (
              ...
              >
                ...
                {estActif && <Icone icone="ph:check-bold" className="w-4 h-4 text-red-600 shrink-0" />}
              </button>
            );
          })}
        </div>
